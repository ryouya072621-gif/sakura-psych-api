...
# タイプ相性マトリクス（基本スコア）- 厳しめ設定
# D: 導型, S: 和型, C: 理型, P: 陽型
TYPE_COMPATIBILITY_MATRIX = {
    ("D", "D"): 55,  # 同タイプ: 似すぎて衝突しやすい
    ("D", "S"): 75,  # 導×和: リーダーとサポーターで好相性
    ("D", "C"): 65,  # 導×理: 論理的な補佐役として機能
    ("D", "P"): 45,  # 導×陽: 両方主張が強く衝突しやすい
    ("S", "D"): 75,
    ("S", "S"): 60,  # 同タイプ: 穏やかだが決断力に欠ける
    ("S", "C"): 80,  # 和×理: 最も安定した組み合わせ
    ("S", "P"): 55,  # 和×陽: 陽がリードしすぎる傾向
    ("C", "D"): 65,
    ("C", "S"): 80,
    ("C", "C"): 55,  # 同タイプ: 慎重すぎて進まない
    ("C", "P"): 70,  # 理×陽: 分析と発信のバランス
    ("P", "D"): 45,
    ("P", "S"): 55,
    ("P", "C"): 70,
    ("P", "P"): 50,  # 同タイプ: 競争的になりやすい
}
...
def calculate_compatibility(data1: dict, data2: dict) -> dict:
    ...
    # 距離による調整（近いほど高スコア）
    # 距離0（同一人物）で100点、距離が大きいほど減点
    # 距離0 → +50, 距離50 → +0, 距離100 → -50
    distance_adjustment = 50 - distance

    # 最終スコア = ベーススコア(45-80) + 距離調整(-50〜+50)
    # 同一人物: 例えばS-Sなら 60 + 50 = 110 → 100（上限）
    final_score = type_score + distance_adjustment
    ...
    return {
        ...
        "distance": round(distance, 2),
        "adjustment": round(distance_adjustment, 1),
        "type1": type1,
        "type2": type2,
        "comments": comments
    }
